...
public class InMemoryJSONView extends Main {
	...
	public InMemoryJSONView(@Autowired RestClientService service) {
		...
		postsGrid.addColumn(node -> node.get("userId")).setHeader("User ID");
		postsGrid.addColumn(node -> node.get("title")).setHeader("Title");
		postsGrid.addColumn(node -> node.get("body")).setHeader("Body");
